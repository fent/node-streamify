...
var Streamify = module.exports = function Streamify(options) {
  ...
  addProp(properties, 'queueMethods');
...
Streamify.prototype.resolve = function(stream) {
  ...
  var self = this;
  hashish(this.__queuedMethods).forEach(function(queue, method) {
    self.__proxy(method);
 ...
 */
Streamify.prototype.queueMethods = function() {
  this.__queuedMethods = {};
  ...
  Object.keys(self.__options.superCtor.prototype).forEach(function(key) {
    if (typeof self.__options.superCtor.prototype[key] === 'function') {
      self.queueMethod(key);
    }
  });

  // queue more methods from the stream spec
  this.queueMethod('destroy', {
    before: function() {
      this.readable = false;
      this.writable = false;
    }
  ...
  if (this.__options.writable) {
    this.queueMethod('write', { returnValue: false });
    this.queueMethod('end');
  }

  if (this.__options.readable) {
    this.queueMethod('setEncoding');
    this.queueMethod('pause');
    this.queueMethod('resume');
  }
};
